...
clear; clc;
Aluno = 'Frederico Rangel Sader';    // Incluir seu NOME aqui
...
function graficar3d()
   ...
   x = 0:2:5*%pi;    // Se quiser, pode mudar o incremento para  0.01
   y = 0:2:2*%pi;    //            " 
   z = 2*cos(x')*cos(y+5);
...
function graficar2d()
   delete(gca()); // limpa a janela gráfica
   a = 0:0.01:4*%pi;    // Se quiser, pode mudar o incremento para  0.01
   plot(a,2*sin(a),a,5*cos(a));  // figura 2D
...
function ASCV_Sobre()
	msg = msprintf(gettext(" Interface desenvolvida na UENF (CCT - LCMAT - CC)"...
                             +"\nAutor: Frederico Rangel Sader,"...
                             +"\nCopyright (C) 2008-2023 FRS-UENF "));
